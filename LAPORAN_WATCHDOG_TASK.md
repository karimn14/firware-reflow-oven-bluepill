...
**Total stack statis:** 960 word = 3.840 byte untuk enam task aplikasi; 1.056 word = 4.224 byte termasuk idle task. Angka ini adalah kapasitas yang dialokasikan, bukan konsumsi stack puncak yang terukur. `configCHECK_FOR_STACK_OVERFLOW=2` aktif, tetapi firmware belum merekam `uxTaskGetStackHighWaterMark()` sehingga konsumsi maksimum tiap task belum diketahui. Prioritas FreeRTOS lebih tinggi bila angkanya lebih besar. TCB dan variabel statis lain tidak termasuk total stack ini.
